  ...
  if (LOCAL_SPLIT_U > 1) {
    ...
    for (int i=0; i<LOCAL_SPLIT_U; i++) {
        ...
        VecCpy<GLOB_STORE_WIDTH>(&regC[i * GLOB_STORE_TOTAL_WIDTH + 
         ...
         if (i > 0) {
          ...
          for (int k=0; k<GLOB_STORE_WIDTH; k++) {
            regC[j * GLOB_STORE_WIDTH + k] += regC[i * GLOB_STORE_TOTAL_WIDTH + j * GLOB_STORE_WIDTH + k];
          }
      }
      }
      if (GLOB_STORE_REAR_WIDTH) {
        ...
        if (i > 0) {
          #pragma unroll
          for (int k=0; k<GLOB_STORE_REAR_WIDTH; k++) {
            regC[GLOB_STORE_NUM * GLOB_STORE_WIDTH + k] += regC[i * GLOB_STORE_TOTAL_WIDTH + GLOB_STORE_NUM * GLOB_STORE_WIDTH + k];
          }
      }
      }
    }
