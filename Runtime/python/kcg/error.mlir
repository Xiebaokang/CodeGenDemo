  ...
  func.func public @GEMM_mnk1024x1024x512_f32f32f32_TTmn4x4_BTmnk64x64x16(%gA: memref<512x1024xf32, 1>, %gB: memref<512x1024xf32, 1>, %gC: memref<1024x1024xf32, 1>) attributes {func.op.name = "Matmul", func.state = "gpu", nvvm.kernel = 1 : i32} {
    affine.parallel (%BX, %BY) = (0, 0) to (16, 16) {  // bx,by
      ...
      affine.parallel (%TX, %TY) = (0, 0) to (16, 16) {  // tx,ty
        ...
        %TMPA = memref.alloca() {alignment = 16 : i64} : memref<4xf32, 5>  // temp A
        ...
        affine.for %iBK = 0 to 4 {
          affine.for %arg8 = 0 to 4 {
            ...
            affine.store %cst, %regC[%iBK, %arg8] : memref<4x4xf32, 5>
          } {affine.loop = "unroll"}
        } {affine.loop = "unroll"}
        // A[tx * 4 + ty floordiv 4 + 0 * 64 + bx * 64, (ty mod 4) * 4 + 0]
        // BK=16,16/4个float=4,所以一行需要4次vecload即可。所以用 %TY floordiv 4, thread按顺序依次搬运 gA
        // 当a transpose后，需要变下索引计算(shmA形状变为 BK行BM列。vecload ：[%TX + %TY floordiv(BM/4),(%TY mod (BM/4)) * 4]  )
        // %4 = affine.vector_load %gA[ERROR: %TX * 4 + %TY floordiv 4 + %c0 * 64 + %BX * 64, (%TY mod 4) * 4 + %c0_4] : memref<512x1024xf32, 1>, vector<4xf32>  // load A
        %4 = affine.vector_load %gA[%BX * 64 + %TX + %TY floordiv 16,(%TY mod 64) * 4] : memref<512x1024xf32, 1>, vector<4xf32>  // load A
        affine.vector_store %4, %TMPA[%c0 * 4] : memref<4xf32, 5>, vector<4xf32>  // A->tmpA
        ...
        affine.vector_store %5, %TMPB[%c0 * 4] : memref<4xf32, 5>, vector<4xf32>  // tmpB
        // modify index
        // affine.for %iBK = 0 to 4 {
        ...
        // } {affine.loop = "unroll"}
        %9 = affine.vector_load %TMPA[%TX + %TY floordiv 16] : memref<4xf32, 5>, vector<4xf32>  // read tmpa
        ...
        affine.for %iBK = 0 to 512 step 16 {
          ...
          affine.for %arg8 = 0 to 15 {
            %11 = affine.vector_load %SHMA[(%iBK floordiv 16) mod 2, %arg8 + 1, (((%TX * 16 + %TY) mod 64) floordiv 16 + ((%TX * 16 + %TY) floordiv 64 + %c0 * 4) * 4) * 4] : memref<2x16x64xf32, 3>, vector<4xf32>
            affine.vector_store %11, %regA[(%arg8 + 1) mod 2, %c0 * 4] : memref<2x4xf32, 5>, vector<4xf32>
            %12 = affine.vector_load %SHMB[(%iBK floordiv 16) mod 2, %arg8 + 1, (%TY mod 16 + %c0 * 16) * 4] : memref<2x16x64xf32, 3>, vector<4xf32>
